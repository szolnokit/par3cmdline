...
// Gaussian elimination for Sparse Random Matrix (16-bit).
...
static int rs16_gaussian_sparse(PAR3_CTX *par3_ctx, int lost_count)
{
	...
	matrix = malloc(sizeof(uint16_t) * (size_t)block_count * avail);
	if (matrix == NULL){
	...
	for (y = 0; y < avail; y++){
		for (x = 0; x < block_count; x++){
			matrix[block_count * y + x] = (uint16_t)sparse_matrix_element(par3_ctx, x, all_ids[y]);
		}
	}
	...
	for (y = 0; y < lost_count; y++){
		...
		if (r != y){	// Swap rows and their recovery ids.
			...
			all_ids[r] = tmp_id;
			for (x = 0; x < block_count; x++){
				uint16_t tmp = matrix[block_count * y + x];
		...
		factor = gf16_reciprocal(gf_table, matrix[block_count * y + pivot]);
...
int rs16_gaussian_elimination(PAR3_CTX *par3_ctx, int lost_count)
{
	...
	if (par3_ctx->ecc_method & 2)	// Sparse Random Matrix
		return rs16_gaussian_sparse(par3_ctx, lost_count);
